...
public class StockAccount {
    ...
    public void enterInform() {
		...
		static void buy(double amount,String sharename) {
			for (int j = 0; j < stock.size(); j++) {
				Stock p = stock.get(j);
				if (sharename.equals(p.getShareName())) {
            double shareamount = p.getSharePrice() * p.getShareNumber();
    ...
    public void stockReport() {
		...
		for(int i=0;i<stock.size();i++) {
			Stock s=stock.get(i);
			double shareprice=s.getSharePrice() *s.getShareNumber();
			total +=shareprice;
			System.out.println("stock name = "+s.getShareName()+"\nnumber of shares = "+s.getShareNumber()+"\nstock price = "+s.getSharePrice()+"\nTotal price of stock = "+total+"\n");
		}
